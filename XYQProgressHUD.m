...
#import "XYQProgressHUD.h"
	...
	#define XYQ_AUTORELEASE(exp) exp
	#define XYQ_RELEASE(exp) exp
	#define XYQ_RETAIN(exp) exp
#else
	#define XYQ_AUTORELEASE(exp) [exp autorelease]
	#define XYQ_RELEASE(exp) [exp release]
	#define XYQ_RETAIN(exp) [exp retain]
#endif

#if __IPHONE_OS_VERSION_MIN_REQUIRED >= 60000
    #define XYQLabelAlignmentCenter NSTextAlignmentCenter
#else
...
@property (atomic, XYQ_STRONG) UIView *indicator;
@property (atomic, XYQ_STRONG) NSTimer *graceTimer;
@property (atomic, XYQ_STRONG) NSTimer *minShowTimer;
@property (atomic, XYQ_STRONG) NSDate *showStarted;
@property (atomic, assign) CGSize size;

@end


@implementation XYQProgressHUD {
	BOOL useAnimation;
...
+ (XYQ_INSTANCETYPE)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	XYQProgressHUD *hud = [[self alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
	return XYQ_AUTORELEASE(hud);
}

+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    if (view == nil) view = [[UIApplication sharedApplication].windows lastObject];
	XYQProgressHUD *hud = [self HUDForView:view];
	if (hud != nil) {
...
+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    if (view == nil) view = [[UIApplication sharedApplication].windows lastObject];
	NSArray *huds = [XYQProgressHUD allHUDsForView:view];
	for (XYQProgressHUD *hud in huds) {
		hud.removeFromSuperViewOnHide = YES;
		[hud hide:animated];
	}
	return [huds count];
...
+ (XYQ_INSTANCETYPE)HUDForView:(UIView *)view {
	NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
	for (UIView *subview in subviewsEnum) {
		if ([subview isKindOfClass:self]) {
			return (XYQProgressHUD *)subview;
		}
	}
	return nil;
}
...
- (void)showWhileExecuting:(SEL)method onTarget:(id)target withObject:(id)object animated:(BOOL)animated {
	...
	targetForExecution = XYQ_RETAIN(target);
	objectForExecution = XYQ_RETAIN(object);	
...
- (void)setupLabels {
	...
	label.textAlignment = XYQLabelAlignmentCenter;
	label.opaque = NO;
	...
	detailsLabel.textAlignment = XYQLabelAlignmentCenter;
